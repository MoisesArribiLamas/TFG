...
namespace Es.Udc.DotNet.TFG.Model.Daos.SuministraDao.Tests
{
    ...
    public class SuministraDaoEntityFrameworkTests
    {
        ...
        public void MostrarAhorroXUbicacionPorFechaEnDiasTest()
        {
            ...
            List<AhorroPorDias> consumoResult = suministraDao.MostrarAhorroXUbicacionPorFechaEnDias(u.ubicacionId, fecha1, fecha2);


            Assert.AreEqual(consumoResult[0].Count, 100 + 1000); //c2+c3
            Assert.AreEqual(consumoResult[0].fecha, fecha1);

            Assert.AreEqual(consumoResult[1].Count, 10000); //c4
